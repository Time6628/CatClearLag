...
public class ItemClearingWarning implements Runnable {
    ...
    @Override
    public void run() {
        plugin.getGame().getServer().getBroadcastChannel().send(Text.builder().append(plugin.getPrefix()).color(TextColors.RED).append(Text.of("Ground items will be cleared in " + seconds + " seconds.")).build());
    }
}
